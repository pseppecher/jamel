...
public class RelativePrices extends ChartPanel {
	...
	private static JFreeChart newChart() {
		TimeChart chart = new TimeChart("Relative Prices", "",
				Circuit.getCircuit().getTimeSeries().get(Labels.relativePrices)				
				);
		chart.setColors( 0,JamelColor.averageColor);
		chart.addLineLegendItem("Final Price to Intermediate Price Ratio", JamelColor.averageColor);
		return chart;
	}
