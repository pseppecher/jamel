...
public class Workers extends ChartPanel {
	...
	private static JFreeChart newChart() {
		TimeChart chart = new TimeChart("Workers", "Workers",
				Circuit.getCircuit().getTimeSeries().get(Labels.workersInSector1),
				Circuit.getCircuit().getTimeSeries().get(Labels.workersInSector2)
				);
		chart.setColors(0, JamelColor.intermediateColor, JamelColor.finalColor);
		chart.addLineLegendItem("Sector 1", JamelColor.intermediateColor);
		chart.addLineLegendItem("Sector 2", JamelColor.finalColor);
		return chart;
	}
