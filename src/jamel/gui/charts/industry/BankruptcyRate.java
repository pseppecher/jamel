...
public class BankruptcyRate extends ChartPanel {
	...
	private static JFreeChart newChart() {
		TimeChart chart = new TimeChart("Bankruptcy Rates", "Percent",
				Circuit.getCircuit().getTimeSeries().get(Labels.annualBankruptcyRateIntermediate),
				Circuit.getCircuit().getTimeSeries().get(Labels.annualBankruptcyRateFinal)
				);
		chart.setColors( 0,JamelColor.intermediateColor,JamelColor.finalColor,JamelColor.averageColor);
