 ...
 * It is generally used to describe those working for a single company or industry, 
 * but can also apply to a geographic region like a city, state, country, etc. The 
 * term generally excludes the employers or management, and can imply those 
 * involved in manual labour."
 * (<a href="https://en.wikipedia.org/wiki/Workforce">Wikipedia</a>)
 */
...
public class Workforce extends LinkedList<JobContract> {
	...
	/**
	 * Returns the payroll.<p>
	 * "In accounting, payroll refers to the amount paid to employees for services 
	 * they provided during a certain period of time."
	 * (<a href="https://en.wikipedia.org/wiki/Payroll">Wikipedia</a>)<p>
	 * The payroll is calculated each time the <code>getPayroll()</code> method is 
	 * called, by summing the wages of each contract in the list.
	 * @return the payroll.
	 */
	...
	public void layoff(int fired) {
		if (fired>this.size()) {
			throw new IllegalArgumentException("The number of workers to be fired exceeds the workforce size.");
		}
