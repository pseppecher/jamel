...
public class BankingSector implements Sector, Asset {
	...
	private class Account implements BankAccount {
		...
		/**
		 * Closes the account. Cancels all the loans associated with this
		 * account. Called in case of a bankruptcy.
		 */
		private void close() {
			if (!open) {
		...
		public void lend(final long principalAmount) {
			...
			this.loans.add(new Loan() {
...

				{
					...
					this.maturityDate = Circuit.getCurrentPeriod().plus(
							BankingSector.this.p.normalTerm);
					this.extendedDate = Circuit.getCurrentPeriod().plus(
							BankingSector.this.p.extendedTerm);
				...
				public boolean isDoubtfull() {
					return (Circuit.getCurrentPeriod()
							.isAfter(this.maturityDate));
				}
	...
	private static class KEY {
		...
		/**
		 * When the sector send its data for aggregation at the end of the
		 * period.
		 */
	...
	private class Parameters {
		 ...
		 */
		private void update() {
			this.extendedTerm = Integer.parseInt(circuit.getParameter(
					BankingSector.this.getName(), EXTENDED_TERM));
			this.normalTerm = Integer.parseInt(circuit.getParameter(
					BankingSector.this.getName(), TERM));
			this.patience = Integer.parseInt(circuit.getParameter(
					BankingSector.this.getName(), PATIENCE));
			this.penaltyRate = Float.parseFloat(circuit.getParameter(
					BankingSector.this.getName(), PENALTY_RATE));
			this.propensityToDistributeCapitalExcess = Float.parseFloat(circuit
					.getParameter(BankingSector.this.getName(),
							CAPITAL_PROP_TO_DISTRIBUTE));
			this.rate = Float.parseFloat(circuit.getParameter(
					BankingSector.this.getName(), RATE));
			this.targetedCapitalRatio = Float.parseFloat(circuit.getParameter(
					BankingSector.this.getName(), CAPITAL_RATIO));
		}
	...
	private boolean checkConsistency() {
		...
		if (result = true) {
			if (sumDeposit != v.liabilities || sumDebt != v.assets
					|| v.assets - v.liabilities != v.capital) {
				result = false;
			}
		}
		return result;
	...
	/**
	 * Close the bank. The data of the period are computed and forwarded to the
	 * circuit.
	 */
	private void close() {
		this.dataset.update();
		this.circuit.forward(KEY.putData, this.name,
				new RepresentativeAgentDataset(this.dataset));
		if (!checkConsistency()) {
			throw new RuntimeException("Inconsistency");
		}
	}
	...
	private void newOwner() {
		...
		this.bankOwner = (CapitalOwner) BankingSector.this.circuit
				.forward(KEY.selectCapitalOwner);
		if (this.bankOwner != null) {
	...
	public boolean doPhase(String phaseName) {
		...
		if (phaseName.equals("opening")) {
			...
			throw new IllegalArgumentException(this.name + ": Unknown phase <"
					+ phaseName + ">");
		}
