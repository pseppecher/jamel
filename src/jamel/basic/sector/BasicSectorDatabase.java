...
public class BasicSectorDatabase implements SectorDataset {
	...
	/**
	 * Returns a selection of individual datasets.<p>
	 * FIXME: il est probable que cette methode plante si select contient sous-conditions (operateurs and() et or() imbriques).
	 * @param data the data in which the individual 
	 * @param select a String that describes the datasets to be selected (<code>null</code> not permitted).
	 * If <b>select</b> is empty, all datasets are selected.
	...
	private static Map<String,AgentDataset> select(final Map<String,AgentDataset> data, final String select) {
		...
		if (select==null) {
			...
			if (query.startsWith("name=")) {
				...
				final Map<String,AgentDataset> selection = select(data,arg[0]);
				result = select(selection,arg[1]);
			}
	...
	/**
	 * Breaks the specified String in two pieces around the first comma. Commas inside parentheses are ignored.
	 * @param string the string to be split.
	 * @return the array of strings computed by splitting this string around the first comma.
	 */
	private static String[] split(final String string) {
		...
		if (split==-1) {
			throw new IllegalArgumentException("Select: "+ string);					
		}
		result = new String[2];
	...
	/**
	 * Returns an array that contains the sum, the mean, the max and the min of the specified collection of individual data.
	 * @param data the key for the data to analyse.
	 * @param select how to select data.
	 * @return an array that contains the sum, the mean, the max and the min of the specified collection of individual data.
	 */
	...
	@Override
	public void putSectorialData(String key, Double value) {
