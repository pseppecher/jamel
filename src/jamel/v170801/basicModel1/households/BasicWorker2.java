...
import jamel.v170801.basicModel1.firms.Employer;
import jamel.v170801.basicModel1.firms.Goods;
import jamel.v170801.basicModel1.firms.JobOffer;
import jamel.v170801.basicModel1.firms.LaborContract;
 ...
 * Represents a worker.
 */
public class BasicWorker2 extends JamelObject implements Agent, Worker {
	...
	static public Consumer<? super Agent> getAction(final String phaseName) {
		...
		case "jobSearch":
			action = (agent) -> {
				((BasicWorker2) agent).jobSearch();
			};
			break;
		case "consumption":
			action = (agent) -> {
				((BasicWorker2) agent).consumption();
			};
			break;
		default:
