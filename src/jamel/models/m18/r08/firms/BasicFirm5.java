 ...
 * A basic firm.
 */
public class BasicFirm5 extends AbstractFirm {
			...
			if (this.getTotalValue() > 0 && amount > this.size()) {
				for (final Equity title : this.equities) {
					...
					if (newDividend > 1) {
						...
						// newDividend);
						shareholder.acceptDividendCheque(BasicFirm5.this.account.issueCheque(shareholder, newDividend));
						result += newDividend;
						if (result > amount) {
	...
	class WorkforceManager {
		...
		private void close() {
			BasicFirm5.this.putData(keys.workforce, workforce.size());
			BasicFirm5.this.putData(keys.vacancies, this.vacancies);
			BasicFirm5.this.putData(keys.jobOpenings, this.jobOpenings);
			BasicFirm5.this.putData(keys.vacancyRatio,
		...
		private void payWorkers() {
			...
			BasicFirm5.this.putData(keys.wageBill, wagebill);
		}
		...
		void updateWage() {
			...
			if (this.wage == null) {
				...
				for (int i = 0; i < cons.observations; i++) {
					final BasicFirm5 firm = (BasicFirm5) laborMarket.selectEmployer();
			...
			putData(keys.wageFactor, this.wageFactor);
		}
		...
		void updateWorkforce() {
			...
			BasicFirm5.this.putData(keys.jobOffers, jobOpenings);
			BasicFirm5.this.putData(keys.workforceTarget, manpowerTarget);
	...
	protected void checkSolvency() {
		...
		if (((debt != 0 && debt > assets) || this.factory.getCapacity() == 0) && getPeriod() >= this.cons.supervision) {
	...
	protected void imitation() {
		...
		if (now > cons.supervision) {
			if (now > this.lastImitiation + 12) {
				BasicFirm5 model = (BasicFirm5) sector.select();
				imitation(model);
	...
	protected void imitation(BasicFirm5 model) {
		if (this.imitations != 0 || this.lastImitiation + 12 >= getPeriod()) {
			throw new RuntimeException("Inconsistency");
		}
		this.lastImitiation = getPeriod();
		this.imitations = 1;
