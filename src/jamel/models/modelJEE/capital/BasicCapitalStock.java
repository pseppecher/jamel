 ...
 */
public class BasicCapitalStock extends JamelObject implements CapitalStock {
	 ...
	 * 
	 */
	public BasicCapitalStock(Corporation corporation, Account account, Simulation simulation) {
		super(simulation);
	 ...
	 * 
	 */
	public BasicCapitalStock(Corporation corporation, int shareholders, Account account, Simulation simulation) {
		this(corporation, account, simulation);
	...
	private StockCertificate getNewShares(final int id, final int size) {
		...
		if (size + this.issuedShares > this.sharesAuthorised) {
			throw new IllegalArgumentException(
					"Shares issued: " + (size + this.issuedShares) + ", authorised: " + this.sharesAuthorised);
		}
		...
		return new StockCertificate() {
			...
			@Override
			public Cheque getDividend(AccountHolder shareholder) {
				if (!open) {
				...
				if (amount == null || amount == 0) {
					...
					result = account.issueCheque(shareholder, amount);
		...
		if (period == null) {
			...
			if (period != this.getPeriod()) {
				throw new RuntimeException(
						"Current period expected <" + period + "> but was <" + this.getPeriod() + ">");
			}
		}
		this.open = true;
		this.dividend = null;
	...
	public void setDividend(final long dividend) {
		...
		if (this.issuedShares != this.sharesAuthorised) {
			throw new RuntimeException("Issued shares is " + this.issuedShares + ", expected " + this.sharesAuthorised
					+ ": " + this.corporation.getName() + ", period " + this.getPeriod());
		}
