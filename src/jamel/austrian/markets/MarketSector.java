...
public class MarketSector extends SFCSector {
	 ...
	 * Mind that the markets are zero-indexed, i.e. B1 firms sell in the market
	 * B0.
	 */
	...
	public MarketSector(String name, Circuit aCircuit) {
		...
		markets.put(new Market("interbank", circuit, this));
		markets.put(new Market("repo", circuit, this));
	}

	private double getFixedPrice_C_Index() {
		double index = 0F;
		for (Market market : markets.getList()) {
			if (market.getName().equals("loans"))
				continue;
			if (market.getName().equals("equity"))
				continue;
			if (market.getName().equals("savings"))
				continue;
			if (market.getName().equals("labor"))
				continue;
			if (market.getStage() >= 1)
				continue; // getStage() returns the stage where goods are sold,
							// not where they are produced. Hence, >=1.
	...
	private double getLaspeyres_C_Index() {
		...
		for (Market market : markets.getList()) {
			...
			if (market.getStage() > 1)
				continue;
			if (market.getSales() == 0)
				continue;
			if (market.getLastSales() == 0)
				continue;
	...
	public Phase getPhase(String name) {
		...
		if (name.equals("opening")) {
			...
			result = new AbstractPhase(name, this) {
				@Override
				public void run() {
					while (getHouseholdsSector().getDemanders() + getFirmsSector().getDemanders() != 0) { // The
																											// reason
																											// for
																											// this
																											// increased
																											...
																											// of
																											// production.
						if (getHouseholdsSector().getDemanders() > 0)
							getHouseholdsSector().budgetAllocation(); // Firms
																		// can
																		// now
																		// plan
																		// their
																		// production
																		// just-in-time.
						if (getFirmsSector().getDemanders() > 0)
							getFirmsSector().investment();
					}
				}
			};
		}
				...
				public void run() {
					while (getHouseholdsSector().getDemanders() + getFirmsSector().getDemanders() != 0) {
						if (getFirmsSector().getDemanders() > 0)
							getFirmsSector().investment();
						if (getHouseholdsSector().getDemanders() > 0)
							getHouseholdsSector().budgetAllocation();
					}
				}
			};
		}
