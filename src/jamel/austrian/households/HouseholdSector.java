...
public class HouseholdSector extends SFCSector {
	...
	/** The list of the households with unsaturated demand (as updated during a time period). */
	...
	protected List<Household> createHouseholds(String type, int lim) {
		...
		for(int index=0;index<lim;index++) {
			...
			try {
				list.add((Household) Class.forName(type,false,ClassLoader.getSystemClassLoader()).getConstructor(
						String.class,Circuit.class,SFCSector.class).newInstance(name,circuit,HouseholdSector.this));
			} catch (Exception e) {
	...
	public Phase getPhase(String name) {
		...
		if (name.equals("opening")) {
			...
			result = new AbstractPhase(name, this){
				@Override public void run() {
					if (concurrentHouseholds.getList().size()!=0) throw new RuntimeException("Bad handling of demand.");
	...
	public void budgetAllocation() {
		do{	Household household = concurrentHouseholds.getRandomAgent();
			 if (!household.budgetAllocation()) concurrentHouseholds.remove(household);
		} while (concurrentHouseholds.getList().size()>0);
	}
	

	/**
	 * Adds a household to the list of households with unsaturated demand (after a wage payment has been received).
	 */
	public void registerNewDemand(BasicHousehold household) {
		if (!concurrentHouseholds.contains(household)) concurrentHouseholds.put(household);
	}


	/**
	 * Returns an ownership structure of a firm or bank.
	 * Shareholders are associated with numbers that reflect percentages of ownership.
	 * All shareholders get equal shares.
	 * @param the number of shareholders.
	...
	public HashMap<Shareholder,Number> selectRandomShareholders(int number) {
		...
		while (selection.size()<number){
			...
			if (!selection.contains(randomHousehold)) selection.add(randomHousehold);
			}
		...
		if (ownershipStructure.size()!=number) throw new RuntimeException("False definition of ownership.");
