 ...
 * <p>
 */
public class CommercialBank extends SFCAgent implements Bank {
		
	/** The bank's own account (own funds). */
	private final Account reserves;
	
	/** The loanable funds. */
	private final Account loanableFunds;

	/** The list of the customers' checking accounts. */
	private final HashMap<AccountHolder,Account> checkingAccounts;
	 ...
	 */
	public CommercialBank(String aName, Circuit aCircuit, BankingSector aSector, Float lendingRate, Float savingsRate, Integer money) { 
		super(aName, aCircuit, aSector);
		reserves = new Account(this);
		reserves.credit(money);
		subscribedCapital = money;
		loanableFunds = new Account(this);
		checkingAccounts = new HashMap<AccountHolder, Account>() ;
	 ...
	 */
	public CommercialBank(InvestmentProject project, Circuit aCircuit,BankingSector aSector, Float lendingRate, Float savingsRate) { 
		super("Bank"+project.getSpecificID(), aCircuit, aSector);
		reserves = new Account(this);
		Cheque cheque = sector.getInvestmentBank().underwrite(project.getID());
	 ...
	 */
	public Cheque newCheque(AccountHolder customer, int amount){
		if (!checkingAccounts.containsKey(customer)) throw new RuntimeException("You don't have an account here.");
		return checkingAccounts.get(customer).newCheque(amount);
	}
	 ...
	 */
	public void deposit(AccountHolder customer, Cheque cheque){
		if (!checkingAccounts.containsKey(customer)) throw new RuntimeException("You don't have an account here.");
	...
	public int getBalance(AccountHolder customer){
		if (!checkingAccounts.containsKey(customer)) throw new RuntimeException("You don't have an account here.");
		return checkingAccounts.get(customer).getAmount();		
	}
	...
	public void closeAccount(AccountHolder customer){
		if (!checkingAccounts.containsKey(customer)) throw new RuntimeException("You don't have an account here.");
		if (checkingAccounts.get(customer).getAmount()>0) throw new RuntimeException("You still have money.");
	...
	private int setTargetCreditVolume(){
		...
		if (targetedLendingVolume >= loanableFunds.getAmount()){
			targetedLendingVolume = loanableFunds.getAmount();
		}
	...
	public Cheque acceptDebtor(CreditContract newContract){
		...
		Cheque cheque = loanableFunds.newCheque(newContract.getVolume());
	...
	private void setSavingsRate(){
		...
		for (CreditContract loan:loanBook){
			...
			for (int t=0;t<duration;t++){ //TODO: maybe use volume and interpret it as prospective average profits.
				averageRevenue += loan.getFaceValue()*loan.getInterest()*(1-t/duration); //formula simplified.
			}			
		}
		float averageCost = 0;
	...
	public void payInterest() {
		...
		excessLoanableFunds = outstandingLoans + loanableFunds.getAmount() - timeDepositVolume;
		if (excessLoanableFunds>0){
			if (loanableFunds.getAmount()>=excessLoanableFunds){
				loanableFunds.debit(excessLoanableFunds);
				reserves.credit(excessLoanableFunds);
			}
			else{
				reserves.credit(loanableFunds.getAmount());
				loanableFunds.reset();
			}
		}
		...
		int reserves = this.reserves.getAmount();
		...
		for ( TimeDeposit deposit : interestObligations ) {
			...
			if (interestPayment>0) {
				Cheque cheque = this.reserves.newCheque(interestPayment);
		...
		if (illiquid==true & this.reserves.getAmount()!=0) throw new RuntimeException("False computation of payments");
		...
		int loanableFunds = this.loanableFunds.getAmount();
		...
		if (loanableFunds < totalRedemptionObligations){
			if (this.reserves.getAmount()>=totalRedemptionObligations-loanableFunds){
				...
				illiquid = true;
				debtorQuality = Quality.BAD;
				this.loanableFunds.credit(this.reserves.getAmount());
				this.reserves.reset();
				loanableFunds = this.loanableFunds.getAmount();
		...
		for ( TimeDeposit deposit : redemptionObligations ) {
			...
			if (redemptionPayment>0) {
				Cheque cheque = this.loanableFunds.newCheque(redemptionPayment);
		...
		if (creditorShare_2 < 1 & this.reserves.getAmount()!=0) throw new RuntimeException("False computation of payments");
		if (creditorShare_2 < 1 & this.loanableFunds.getAmount()!=0) throw new RuntimeException("False computation of payments");
	...
	private void clearing(){
		...
		if (fundingGap>0){
			if (fundingGap<=reserves.getAmount()){
				reserves.debit(fundingGap);
				loanableFunds.credit(fundingGap);
				fundingGap = 0;
			}
			else{
				fundingGap -= reserves.getAmount();
				loanableFunds.credit(reserves.getAmount());
				reserves.reset();
			}
		}
		
		if (writedowns>0){
			if (writedowns<=reserves.getAmount()){
				reserves.debit(writedowns);
				loanableFunds.credit(writedowns);
			}
			else{
				fundingGap += writedowns-reserves.getAmount();
				loanableFunds.credit(reserves.getAmount());
				reserves.reset();
			}
		}
	...
	public void updateProfits(){
		...
		if (reserves.getAmount() > requiredReserves) dividend = reserves.getAmount()-requiredReserves;
		else dividend = 0;
		...
		if (dividend > 0){
			...
			for (Shareholder owner:owners.keySet()){
				...
				if (dividend>0) owner.receiveDividend(reserves.newCheque(dividend ));
			...
			if (dividendRemainder>0) {
				...
				while (dividendRemainder>0){
					copyOfOwners.removeFirst().receiveDividend(reserves.newCheque(1));
					dividendPaid+=1;
					dividendRemainder-=1;
				}
			}
		...
		if (debtorQuality == Quality.BAD){
			if (loanBook.size()==0){
				...
				if (savingsAccounts.size()>0 && loanableFunds.getAmount()+reserves.getAmount() == 0){//The bank cannot pay its obligations.
					...
					((BankingSector) sector).bankruptcy(this);
					isbankrupt = true;
				}
				else if (loanableFunds.getAmount() + reserves.getAmount()==0){	//The bank has no capital to resume business.
					((BankingSector) sector).bankruptcy(this);
					isbankrupt= true;
	...
	/**
	 * Closes the bank and sets the credit target for the next period.<br>
	...
	public void close() {	
		...
		equity = reserves.getAmount()
				+loanableFunds.getAmount()
				+loans
				-timeDepositVolume;
	 ...
	 * Gets the data from each household and adds them to the sector data.
	 */
	private void updateData() {
		...
		data.put("moneyHoldings" , (double) loanableFunds.getAmount()+reserves.getAmount());
		...
		data.put("loanableFunds" , (double) loanableFunds.getAmount());
		data.put("reserves" , (double) reserves.getAmount());
	...
	public void terminateOperations() {
		...
		do otherBank = ((BankingSector) sector).selectRandomBank(); while (otherBank==this);
		for (AccountHolder accountHolder : checkingAccounts.keySet()){
			...
			int deposit = checkingAccounts.get(accountHolder).getAmount();
			if (deposit>0){
				Cheque transfer = checkingAccounts.get(accountHolder).newCheque(deposit);
