...
import jamel.v170804.util.Amount;
import jamel.v170804.util.BasicAmount;
...
public class BasicBank extends JamelObject implements Agent, Bank {
	...
	private abstract class AbstractCheque extends Amount implements Cheque {
		...
		public boolean isValid() {
			return (this.issue != null) && (this.issue == getPeriod());
		}
		...
		private class Debt {
			...
			private class BasicLoan extends Amount implements Loan {
				...
				private BasicLoan(final long amount, final int term, final boolean amortizing) {
					...
					this.plus(amount);
					this.loanRate = BasicBank.this.rate;
				...
				public void repay() {
					...
					BasicBank.this.checkConsistency();
					Jamel.println();
					Jamel.println("Before");
					Jamel.println("BasicBank.this.installments",BasicBank.this.installments);
					Jamel.println("Debt.this.installments",Debt.this.installments);
					Jamel.println("BasicBank.this.interests",BasicBank.this.interests);
					Jamel.println("Debt.this.interests",Debt.this.interests);
					*/
			...
			private class OverdueDebt extends Amount implements Loan {
				...
				public void repay() {
					// TODO Check une fois et une seule par période.
					final long interest = (long) (this.getAmount() * BasicBank.this.penaltyRate);
					this.plus(interest);
			...
			private void cancel(long writeOff) {
				...
				ArgChecks.negativeOr0NotPermitted(writeOff, "toBeCancelled");
	...
	public void close() {
		...
		this.dataset.put(keys.assets, this.outstandingDebtAmount.getAmount() + this.overdueDebtAmount.getAmount());
		this.dataset.put(keys.liabilities, this.depositsAmount.getAmount());
		this.dataset.put(keys.installments, this.installments);
		this.dataset.put(keys.interests, this.interests);
		this.dataset.put(keys.overdueDebt, this.overdueDebtAmount.getAmount());
		this.dataset.close();
