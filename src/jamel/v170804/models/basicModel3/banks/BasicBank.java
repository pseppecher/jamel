...
public class BasicBank extends JamelObject implements Agent, Bank {
	...
	final private List<LoanAccount> loanAccounts = new ArrayList<>();
	...
	private void debtRecovery() {
		...
		for (int i = 0; i < this.loanAccounts.size(); i++) {
			final LoanAccount loanAccount = this.loanAccounts.get(i); 
			loanAccount.debtRecovery(this.penaltyRate);
			if (!loanAccount.isSolvent()) {
				loanAccount.bankrupt();
				checkConsistency();
			}
		}
		// checkConsistency();
	 ...
	 * 
	 * @param account
	 *            a basic account that will be linked with this loan account.
	 * 
	 * @return a new loan account.
	 */
	LoanAccount getNewLoanAccount(BasicAccount account) {
		final LoanAccount result = new LoanAccount(account, this.outstandingLoans);
		this.loanAccounts.add(result);
		return result;
	}
