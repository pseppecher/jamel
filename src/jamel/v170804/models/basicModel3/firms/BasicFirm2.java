 ...
 * A basic firm.
 */
public class BasicFirm2 extends JamelObject implements Firm {
	...
	static public Consumer<? super Agent> getAction(final String phaseName) {
		...
		case "planProduction":
			action = (agent) -> {
				((BasicFirm2) agent).planProduction();
			};
			break;
		case "production":
			action = (agent) -> {
				((BasicFirm2) agent).production();
			};
			break;
		case "payWages":
			action = (agent) -> {
				((BasicFirm2) agent).payWages();
			};
			break;
		case "payDividends":
			action = (agent) -> {
				((BasicFirm2) agent).payDividends();
			};
			break;
		default:
	 ...
	 *            the id of this firm.
	 */
	public BasicFirm2(final Sector sector, final int id) {
